...
function loadComponents() {
    ...
    const authLink = `<div style="position: relative; display: flex; gap: 6px; align-items: center; margin-left: 0.5rem;">
             <div id="loginBtn" style="display: none; gap: 6px;">
                 <button onclick="showLoginModal()" class="btn-small" style="margin-top: 0; padding: 0.4rem 0.8rem; text-decoration: none; font-size: 0.85rem; white-space: nowrap; border: none; cursor: pointer; background: var(--primary); color: white; font-family: inherit;">Login</button>
             ...
             <img id="profileIcon" src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/person.svg" alt="Profile" style="display: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; object-fit: cover; border: 2px solid var(--primary);">
...
function completeAllProgress() {
    ...
    // 1. Fetch the registry of courses first to know what exists
    fetch(`${root}courses.json`)
        .then(res => res.json())
        .then(courses => {
            // 2. Fetch each course individually, handling errors gracefully
            const promises = courses.map(course => 
                fetch(`${root}${course.id}.json`)
                    .then(res => {
                        if (!res.ok) throw new Error(`Missing ${course.id}.json`);
                        return res.json();
                    })
                    .catch(err => {
                        console.warn(`Skipping ${course.id}:`, err);
    ...
    if (window.loginWithEmail) {
        ...
        const interval = setInterval(() => {
            ...
            if (window.loginWithEmail) {
                ...
                window.handleEmailLogin(); // Retry
            } else if (attempts > 10) {
                clearInterval(interval);
                if(btn) { btn.textContent = originalText; btn.disabled = false; }
                alert("Authentication system failed to load. Please check your connection.");
            }
        }, 500);
    }
};
        ...
        if (btn) {
            ...
            const interval = setInterval(() => {
                ...
                if (window.loginWithGoogle) {
                    ...
                    btn.disabled = false;
                    window.handleGoogleLogin(); // Retry
                } else if (attempts > 10) {
                    clearInterval(interval);
                    btn.innerHTML = originalHTML;
                    btn.disabled = false;
...
window.handlePageLogin = function() {
    ...
    if (window.loginWithEmail) {
        ...
        const interval = setInterval(() => {
            ...
            if (window.loginWithEmail) {
                clearInterval(interval);
                window.handlePageLogin();
            } else if (attempts > 10) {
                clearInterval(interval);
                btn.textContent = originalText;
                btn.disabled = false;
                alert("Authentication system failed to load.");
            }
        }, 500);
    }
};

// --- Global Auth Script Injection ---
(function() {
    const path = window.location.pathname.toLowerCase();
    const isTutorial = path.includes('/tutorials/');
    const isAssets = path.includes('/assets/js/');
    const root = isTutorial || isAssets ? '../../' : '';
    const src = root + 'auth/firebase-auth.js';
    if (!document.querySelector(`script[src="${src}"]`)) {
        const script = document.createElement('script');
        script.type = 'module';
        script.src = src;
